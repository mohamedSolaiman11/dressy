...
export async function POST(request: Request) {
  ...
  if (!activeAtelier) {
    return NextResponse.json(
      { message: "اختاري أو أضيفي فرع أول عشان تحفظي الفساتين." },
      { status: 409 }
    );
  }
  ...
  if (error || !data) {
    const message =
      error?.code === "23505" ? "كود الفستان موجود قبل كده." : "تعذر حفظ بيانات الفستان.";

    return NextResponse.json({ message }, { status: 500 });
  }

  try {
    await syncDressImages(supabase, data.id, activeAtelier.id, parsed.imageItems);
  } catch {
    return NextResponse.json(
      { message: "اتحفظ الفستان لكن حصلت مشكلة في حفظ الصور." },
      { status: 500 }
    );
  }

  revalidatePath("/dresses");
  revalidatePath("/dresses/new");

  return NextResponse.json({
    id: data.id,
    message: "تم حفظ الفستان."
  });
}
